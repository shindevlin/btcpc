...
//!
//! Given a BTCPC mnemonic (via BTCPC_MNEMONIC env var), derive each role keypair
...
fn run() -> Result<()> {
    ...
    let mnemonic = std::env::var("BTCPC_MNEMONIC")
        .map_err(|_| anyhow!("set BTCPC_MNEMONIC env var (the 12/24-word phrase). It is never printed."))?;
