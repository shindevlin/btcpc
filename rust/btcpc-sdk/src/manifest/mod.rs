//! BTCPC integration manifest — the self-updating contract between the chain
//! and every repo in the ecosystem.
//!
//! # The problem this solves
//!
//! A repo that consumes BTCPC (Bullship, a bot, a service worker) hard-codes an
//! understanding of BTCPC's surface: which routes exist, which ledger entries it
//! submits, the exact fields it must sign. When BTCPC ships a commit that adds,
...
//!      consumer repo's `BTCPC.md` + `BTCPC.lock` and prints the changelog,
...
//! from REMOVED (a hard break). A consumer knows, per BTCPC commit, exactly what
...
pub const CONSUMER_DOC: &str = "BTCPC.md";
pub const CONSUMER_LOCK: &str = "BTCPC.lock";
