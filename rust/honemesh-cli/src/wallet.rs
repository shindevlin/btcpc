...
use honemesh_sdk::{KeyPair, Wallet, WalletFile};
...
pub fn cmd_wallet_create(account: &str, output: Option<&Path>) -> Result<()> {
    ...
    println!("{} {}", "HoneMesh posting pubkey:".bold(), pubkey);
    println!("{}", "HoneMesh role keys:".bold());
...
pub fn cmd_wallet_show(wallet_file: Option<&Path>) -> Result<()> {
    ...
    println!("{} {}", "HoneMesh posting pubkey:".bold(), wf.hone_public_key_hex);
    ...
    let mut addresses: Vec<(String, String, String)> = wf.chain_addresses
        ...
        .map(|(chain, addr)| {
            let dpath = match chain.as_str() {
                ...
                "hone"   => honemesh_sdk::paths::HONE_POSTING_STR,
                _         => honemesh_sdk::paths::HONE_POSTING_STR,
            };
            (chain.clone(), addr.clone(), dpath.to_string())
        })
        .collect();
    // Also include the HoneMesh pubkey itself.
    addresses.push(("hone".into(), wf.hone_public_key_hex.clone(), honemesh_sdk::paths::HONE_POSTING_STR.into()));
...
pub fn cmd_wallet_api_key_gen(
    ...
    let wallet = honemesh_sdk::Wallet::from_phrase(mnemonic, account)?;
    ...
    // Write/update .honemesh/wallet.env.
    let out_path = output
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| std::env::current_dir().unwrap().join(".honemesh").join("wallet.env"));
    ...
    let content = format!(
        "# HoneMesh wallet — generated by `hone wallet api-key-gen`\n\
    ...
    println!();
    println!("Add to .gitignore:  echo '.honemesh/wallet.env' >> .gitignore");

    Ok(())
}

/// Write `.honemesh/wallet.env` in the current directory (or `output` path).
...
pub fn cmd_wallet_env(wallet_file: Option<&Path>, output: Option<&Path>) -> Result<()> {
    ...
    let content = format!(
        "# HoneMesh wallet — generated by `hone wallet env`\n\
    ...
    println!();
    println!("Next steps:");
    println!("  echo '.honemesh/wallet.env' >> .gitignore");
...
fn resolve_wallet_path(input: Option<&Path>) -> Result<PathBuf> {
    ...
    Ok(PathBuf::from(home).join(".honemesh").join("wallet.json"))
}
