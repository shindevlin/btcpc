...
pub fn cmd_wallet_show(wallet_file: Option<&Path>) -> Result<()> {
    ...
    println!("{} {}", "HoneMesh posting pubkey:".bold(), wf.btcpc_public_key_hex);
...
pub fn cmd_wallet_publish(wallet_file: Option<&Path>, mnemonic: &str) -> Result<()> {
    ...
    let mut addresses: Vec<(String, String, String)> = wf.chain_addresses
        ...
        .map(|(chain, addr)| {
            let dpath = match chain.as_str() {
                ...
                "btcpc"   => honemesh_sdk::paths::HONE_POSTING_STR,
    ...
    addresses.push(("btcpc".into(), wf.btcpc_public_key_hex.clone(), honemesh_sdk::paths::HONE_POSTING_STR.into()));
...
pub fn cmd_wallet_api_key_gen(
    ...
    let content = format!(
        "# HoneMesh wallet — generated by `btcpc wallet api-key-gen`\n\
         # Add this file to .gitignore — it contains your API key\n\
         \n\
         HONE_ACCOUNT={}\n\
         HONE_API_KEY={}\n\
         # HONE_API_URL=https://btcpc.net\n",
        account, api_key,
    );
...
pub fn cmd_wallet_env(wallet_file: Option<&Path>, output: Option<&Path>) -> Result<()> {
    ...
    let content = format!(
        "# HoneMesh wallet — generated by `btcpc wallet env`\n\
