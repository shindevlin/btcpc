//! HoneMesh integration manifest — the self-updating contract between the chain
//! and every repo in the ecosystem.
//!
//! # The problem this solves
//!
//! A repo that consumes HoneMesh (Bullship, a bot, a service worker) hard-codes an
//! understanding of HoneMesh's surface: which routes exist, which ledger entries it
//! submits, the exact fields it must sign. When HoneMesh ships a commit that adds,
...
//! from REMOVED (a hard break). A consumer knows, per HoneMesh commit, exactly what
...
pub const CONSUMER_LOCK: &str = "HoneMesh.lock";
