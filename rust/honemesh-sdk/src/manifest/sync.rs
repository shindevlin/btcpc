//! Consumer-side sync: keep a repo's `HoneMesh.md` + `HoneMesh.lock` current and tell
//! the developer/agent exactly what changed.
//!
//! `HoneMesh.lock` is the machine record: the last manifest this repo synced to
//! (surface hash, version, and the subset of entries/routes this repo declares
//! it uses). `HoneMesh.md` is the human/agent-readable rendering: the current
//! contract plus the changelog since the last sync.
//!
//! Flow (`btcpc sync`):
//!   1. obtain the new manifest (from a path, or fetched from a node),
//!   2. load the old manifest from `HoneMesh.lock` (if any),
//!   3. diff, restricted to the surface this repo uses,
//!   4. rewrite `HoneMesh.md` (contract + changelog) and `HoneMesh.lock`,
...
/// The `HoneMesh.lock` on-disk record.
...
/// `HoneMesh.md` and `HoneMesh.lock`. Preserves the repo's declared `uses_*` sets.
pub fn sync_repo(repo_dir: &Path, new_manifest: &Manifest) -> Result<SyncOutcome> {
    ...
    let prior: Option<LockFile> = if lock_path.exists() {
        ...
        Some(serde_json::from_str(&raw).context("parsing HoneMesh.lock")?)
    } else {
        None
    };
...
/// Render `HoneMesh.md`: the current contract plus the since-last-sync changelog.
fn render_doc(m: &Manifest, diff: Option<&ManifestDiff>, lock: &LockFile) -> String {
    ...
    s.push_str("<!-- HoneMesh.md — how this repo uses HoneMesh. GENERATED by `btcpc sync`.\n");
    s.push_str("     Do not edit the generated block by hand. To change which surface\n");
    s.push_str("     this repo tracks, edit `uses_entries`/`uses_routes` in HoneMesh.lock. -->\n\n");
    s.push_str("# How this repo uses HoneMesh\n\n");
    s.push_str(&format!(
        "- **HoneMesh version:** {}\n- **Chain:** {}\n- **Surface hash:** `{}`\n",
    ...
    if lock.uses_entries.is_empty() && lock.uses_routes.is_empty() {
        s.push_str(
            "_This repo tracks the **entire** HoneMesh surface. To get change alerts \
             for only the parts you use, list them under `uses_entries` / \
             `uses_routes` in `HoneMesh.lock`._\n\n",
        );
    } else {
    ...
    s.push_str(&format!(
        "---\n_Generated from HoneMesh {} surface `{}`. Re-run `btcpc sync` after pulling HoneMesh updates._\n",
        m.btcpc_version,
