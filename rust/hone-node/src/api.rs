...
pub struct AppState {
    ...
    /// Nostr relay transport handle — None if HONE_NOSTR is not enabled.
    pub nostr_handle: Option<crate::nostr_transport::NostrHandle>,
    /// Matrix room transport handle — None if HONE_MATRIX is not enabled.
    ...
    /// I2P datagram transport handle — None if HONE_I2P is not enabled.
    pub i2p_handle: Option<crate::i2p::I2pHandle>,
    /// LoRaWAN transport handle — None if HONE_LORAWAN is not enabled.
...
async fn get_account(
    ...
    Ok(Json(serde_json::json!({
        ...
        "keys":           data["keys"],       // all 6 HONE role keys — public by design
...
async fn get_public_machine_status(State(s): State<AppState>) -> Json<serde_json::Value> {
    ...
    let genesis_ts: u64 = std::env::var("HONE_GENESIS_TIMESTAMP")
...
async fn get_node_info(State(s): State<AppState>) -> Json<serde_json::Value> {
    ...
    let is_miner  = std::env::var("HONE_MINER").map(|v| v == "true" || v == "1").unwrap_or(false);
...
struct AccountCreateBody {
    ...
    /// Account that pays the NAME_REGISTRATION_STAKE (10 HONE). Required unless account is exempt.
    #[serde(default)]
...
fn faucet_amount_hunits(epoch: u64) -> u64 {
    if epoch <= 1_000     { 10_000_000_000 }  // 1 HONE
...
async fn get_staking_requirements(State(s): State<AppState>) -> Json<serde_json::Value> {
    ...
    // Per-role minimum stake in hunits (1 HONE = 10_000_000_000 hunits).
    ...
    let clock    = read_param("clock_min_stake",    5  * 10_000_000_000); //   5 HONE
    let miner    = read_param("miner_min_stake",   10  * 10_000_000_000); //  10 HONE
    let storage  = read_param("storage_min_stake",  2  * 10_000_000_000); //   2 HONE
    let sensor   = read_param("sensor_min_stake",   2  * 10_000_000_000); //   2 HONE
    let service  = read_param("service_min_stake",  3  * 10_000_000_000); //   3 HONE
    let verifier = read_param("verifier_min_stake", 5  * 10_000_000_000); //   5 HONE
    let mempool  = read_param("mempool_min_stake",  2  * 10_000_000_000); //   2 HONE
...
HONE_NODE_ID=$NODE_ID
HONE_GENESIS_TIMESTAMP=$GENESIS_TS
HONE_API_PORT=4242
HONE_P2P_PORT=6942
HONE_MINER=true
HONE_CLOCK=true
...
if [[ -n "$ACCOUNT_PUBKEY" ]]; then
    echo "HONE_ACCOUNT_PUBKEY=$ACCOUNT_PUBKEY" >> "$ENV_FILE"
fi
...
/// Optional body: { "stake": <hunits> }  — defaults to clock_min_stake (5 HONE).
async fn post_clock_self_register(
    ...
    let sk = match s.signing_key.as_ref().as_ref() {
        ...
        None => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({
            "error": "node has no signing key configured (HONE_POSTING_KEY not set)"
        }))),
    };
...
/// GET /download/:filename — serve binary files from $HONE_DATA_DIR/downloads/
...
async fn post_bridge_claim_proof(
    ...
    let oracle_key_hex = std::env::var("HONE_ORACLE_KEY").map_err(|_| (
        StatusCode::INTERNAL_SERVER_ERROR,
        Json(serde_json::json!({"error": "HONE_ORACLE_KEY not configured"})),
    ))?;
    let key_bytes = hex::decode(oracle_key_hex.trim()).map_err(|_| (
        StatusCode::BAD_REQUEST,
        Json(serde_json::json!({"error": "invalid HONE_ORACLE_KEY hex"})),
    ))?;
    let secret_key = SecretKey::from_slice(&key_bytes).map_err(|_| (
        StatusCode::INTERNAL_SERVER_ERROR,
        Json(serde_json::json!({"error": "invalid HONE_ORACLE_KEY bytes"})),
    ))?;
...
async fn post_ton_activate(
    ...
    let relay_tron = std::env::var("HONE_TRON_RELAY_ADDRESS").unwrap_or_default();
    let relay_eth  = std::env::var("HONE_ETH_RELAY_ADDRESS").unwrap_or_default();
