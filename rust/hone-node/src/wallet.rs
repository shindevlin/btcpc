...
/// If HONE_ACCOUNT_PUBKEY env var is set, uses that as the posting key (install-script path).
...
fn derive_all(mnemonic: Mnemonic) -> Result<WalletKeys> {
    ...
    let (post_priv,    post_pub)    = slip10(&seed, &[44|H, HONE_COIN|H, 2|H, 0|H])?;
    let (memo_priv,    memo_pub)    = slip10(&seed, &[44|H, HONE_COIN|H, 3|H, 0|H])?;
    let (hide_priv,    hide_pub)    = slip10(&seed, &[44|H, HONE_COIN|H, 4|H, 0|H])?;
    let (seek_priv,    seek_pub)    = slip10(&seed, &[44|H, HONE_COIN|H, 5|H, 0|H])?;
