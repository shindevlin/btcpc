                    ...
                    if let Some(pos) = buf.windows(4).position(|w| w == b"\r\n\r\n") {
                        ...
                        if !config.device_sn.is_empty() {
                            match auth_key {
                                ...
                                None => {
                                    warn!("NTRIP: no auth key in header — allowing (set HONE_GNSS_DEVICE_SN to enforce)");
                                }
                            }
                        } else if let Some(ref sig) = sign_hex {
                            info!("NTRIP Source-Sign present ({}…) — set HONE_GNSS_DEVICE_SN to enforce", &sig[..16.min(sig.len())]);
                        }
