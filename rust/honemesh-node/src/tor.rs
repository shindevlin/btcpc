...
//!
//! Env vars:
//!   HONE_TOR                 — "true" to activate
//!   HONE_TOR_CONTROL_PORT    — Tor control port (default 9051)
//!   HONE_TOR_CONTROL_PASSWORD — password for HASHEDPASSWORD auth (optional)
...
pub async fn start_hidden_service(chain: &Arc<Chain>, api_port: u16, p2p_port: u16) -> String {
    if std::env::var("HONE_TOR").as_deref() != Ok("true") {
        return String::new();
    }

    let control_port: u16 = std::env::var("HONE_TOR_CONTROL_PORT")
...
async fn try_start(
    ...
    // Authenticate
    let password = std::env::var("HONE_TOR_CONTROL_PASSWORD").unwrap_or_default();
...
mod tests {
    ...
    /// When `HONE_TOR` is not set, `start_hidden_service` must return an empty
    ...
    async fn tor_disabled_returns_empty_string_when_env_unset() {
        ...
        std::env::remove_var("HONE_TOR");
        std::env::remove_var("HONE_TOR_CONTROL_PORT");
        ...
        assert!(
            result.is_empty(),
            "expected empty string when HONE_TOR is unset, got: {:?}",
            result
        );
    }

    /// When `HONE_TOR=true` but the Tor control port is not listening,
    ...
    async fn tor_enabled_but_port_unreachable_returns_empty_string() {
        ...
        std::env::set_var("HONE_TOR", "true");
        std::env::set_var("HONE_TOR_CONTROL_PORT", port.to_string());
