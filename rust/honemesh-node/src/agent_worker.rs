...
//! Verifier-only mode (no task execution): HONE_AGENT_VERIFIER=true
//!
...
async fn verify_submitted_tasks(
    ...
    if std::env::var("HONE_AGENT_VERIFIER").map(|v| v == "true" || v == "1").unwrap_or(false)
        || std::env::var("HONE_AGENT_WORKER").map(|v| v == "true" || v == "1").unwrap_or(false)
    {
        // proceed
    } else {
        return;
...
fn broadcast(
    ...
    if let Ok(data) = serde_json::to_vec(&serde_json::json!({ "entry": entry, "sig": sig })) {
        let _ = cmd_tx.try_send(NetCmd::Broadcast { topic: "hone/entries", data });
    }
}
