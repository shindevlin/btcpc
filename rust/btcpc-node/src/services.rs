...
use btcpc_types::{LedgerEntry, EPOCH_MS, NATIVE_TOKEN};

use crate::chain::Chain;
use crate::net::NetCmd;
use crate::utils::now_ms;

// 5 BTCPC for 30 days of a hosted clock node
...
fn provision_hosted_node(buyer_account: &str) -> anyhow::Result<String> {
    ...
    let out = std::process::Command::new("docker")
        .args([
            ...
            "-e", &format!("BTCPC_ACCOUNT={}", buyer_account),
            "-e", "BTCPC_CLOCK=true",
            "-e", "BTCPC_MINER=false",
            "-e", "BTCPC_STORAGE=false",
            "-e", "BTCPC_CHAIN_ID=btcpc-1",
            "-e", &format!("BTCPC_API_PORT={}", api_port),
            "-e", &format!("BTCPC_P2P_PORT={}", p2p_port),
            "-e", "BTCPC_ANNOUNCE_ADDR=",
