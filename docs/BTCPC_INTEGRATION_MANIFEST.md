...
> **Goal:** any repository that uses BTCPC — Bullship first — should carry a
...
Every repo that uses BTCPC drops in one small file, `BTCPC.md` (and, for
...
Run it in the consumer's CI. Now the repo's understanding of BTCPC is *tested*
...
A coding agent working in Bullship's repo currently has to *rediscover* BTCPC's
API by reading BTCPC's source (which may not even be checked out). With this:

- The agent reads `BTCPC.md` → knows the exact routes, signing fields, and
...
| S3 | `btcpc-sdk sync-manifest` command + the `BTCPC.md` template. |
| S4 | Land `BTCPC.md` in Bullship (first consumer); wire `sync-manifest` into its CI. |
| S5 | As v2.14 hosting entries land, they appear in `hosting` automatically — Bullship's `BTCPC.md` picks them up on next sync. |
