...
**Launch: 2026-07-04 12:00 noon Los Angeles = 19:00 UTC. Timestamp `1783191600000`, chain `btcpc-2`.**
...
`btcpc wallet pubkeys --keystore wallets/<name>.keystore.json`):
```jsonc
{ "genesis_timestamp": 1783191600000, "chain_id": "btcpc-2",
  "accounts": {
...
`rust/btcpc-node/src/config.rs`, `rust/btcpc-node/genesis.json`, `docs/CHAIN_CONSTANTS.md`.
...
- (Optional) once the chain is up, `btcpc wallet backup --keystore <f> --node <url>`
...
```
BTCPC_DATA_DIR=/tmp/gv2-smoke BTCPC_CHAIN_ID=btcpc-2 \
  BTCPC_GENESIS_FILE=$PWD/rust/btcpc-node/genesis.json \
  BTCPC_GENESIS_TIMESTAMP=1783191600000 BTCPC_API_PORT=4299 BTCPC_P2P_PORT=6999 \
...
Fresh `BTCPC_DATA_DIR` (the node refuses to re-init over an existing block 0).
Set the v2 env (chain_id btcpc-2, ts 1783191600000, the account's posting PRIVATE
...
`BTCPC_MINER=false` (ARM hotspot: clock + P2P). Point its bootstrap at the primary.
...
- Wire the node-side `POST/GET /api/keystore/backup` route (Layer 3 server half —
  client already ships as `btcpc wallet backup/restore`).
