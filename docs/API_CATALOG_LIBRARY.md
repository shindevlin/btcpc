# HONE API Catalog Library

## Purpose
`hone-api-catalog` turns `public-apis/public-apis` into a local, versioned HONE library that LLM models, agents, runtimes, and service workers can query internally.

The GitHub repo remains the mother hub. HONE snapshots it, verifies links, records the upstream commit, and serves the result locally. Consensus logic should not call arbitrary third-party APIs directly.

## Rust Crate
Path:

```text
rust/hone-api-catalog
```
...
## Update Flow
1. Keep a local checkout of `https://github.com/public-apis/public-apis` outside the main HONE repo.
2. Periodically `git fetch` and fast-forward it.
3. Run the Rust snapshotter against that checkout.
4. Verify links in batches.
5. Store the signed/hash-addressed snapshot where HONE runtimes and LLM agents can query it.
...
## Reward Boundary
A node should not earn HONE merely for listing an API. Rewards should require work:
