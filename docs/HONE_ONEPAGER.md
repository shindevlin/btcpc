...
HONE is a blockchain where every token is earned by a machine doing something useful. AI inference. Data storage. Sensor reporting. Application hosting. The same GPUs, hard drives, and devices that currently mine worthless hashes instead serve real demand — and get paid for it.
...
| Storage | 15% | Hosting files on HONE-FS |
...
**No burn, ever.** All fees, slashed stakes, and unclaimed rewards flow to `hone_recycle` and are re-earned via future block rewards. The supply is 42,000,000 HONE, forever, in perpetual circulation. HONE is the "No Burn" chain.
...
| Decimals | 10 |
| Genesis reward | ~243 HONE per epoch |
| Epoch duration | 30 seconds |
| Halving | Every ~4 years (same schedule as Bitcoin) |

## Cross-chain bridge

HONE connects to Ethereum, Base, Arbitrum, and Bitcoin via a **lock-and-recycle bridge** — not burn-and-mint. When you wrap HONE to wHONE on Base, your native tokens are locked 1:1. When you unwrap, they're released. No minting, no burning, just transfers between reserve pools.

Each destination chain has a hard cap of 4,200,000 wHONE (10% of native supply), pre-minted at deployment. Bridge liquidity is provided by permissionless LPs who lock HONE with a variable commitment period (30 days to 4 years) and earn fees weighted by their lock duration — the same veCRV model that powers Curve Finance.

## Four-tier finality

HONE state is anchored to external chains for independent verification:
...
**HONE** is the digital labor market. Proof of Compute means the mining itself IS the product. Every token represents real work done — an AI prompt answered, a file stored, a sensor reading verified, an application served.
...
HONE_MINER=yourname HONE_ROLES=clock node bin/hone-all

# Full node (GPU required for mining)
HONE_MINER=yourname node bin/hone-all

# Docker (Windows/Mac)
irm https://honemesh.net/hone-start.ps1 | iex
```
